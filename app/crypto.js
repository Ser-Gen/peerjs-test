/*
 * SHA-256 and HMAC-SHA-256 in plain JS. WebCrypto only exists in a secure context, and joining a room
 * must also work on http://<lan-ip>. The inputs are a few hundred bytes, so speed doesn't matter.
 */
...
const bytesOf = input => (typeof input === 'string' ? encoder.encode(input) : input);

/** SHA-256 of a string (UTF-8) or Uint8Array. */
...
/** HMAC-SHA-256 with a Uint8Array key. */
export function hmac(key, message) {
	...
	outerInput.set(sha256(innerInput), 64);
