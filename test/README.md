...
node test/run.mjs room            # one test: room | editor-sync | voice | pwa | vendor | app | start | desktop | editor | monaco | chat | whiteboard
...
| `voice-test.mjs` | `app/voice.js` with a faked room and the media connections from `dom/fakenet.mjs`: who dials of two members in voice, a device without a microphone joining as a listener, mute both ways, per-member volume, a link that drops and a reload, calls that belong to another tool, the copy of a track the level meter reads (never the stream the `<audio>` element plays), a member marked as one we cannot hear yet (no call, no media through it), the ways a pair without a call finds its way back, and a call given up on by one side only (the other is told with `hangup`, because peerjs doesn't close its end). |
| `vendor-test.mjs` | The vendored bundles fit together: `vendor/editor.js` hands out the Yjs of `vendor/yjs.js` and has no copy of its own, `vendor/monaco.js` has none either and starts `monaco.worker.js` (a classic script) with its icon font inside `monaco.css`, and pdf.js (`vendor/pdf.js` with its worker) opens a PDF and reads its text. |
...
| `dom/monaco-test.mjs` | The Editor with Monaco chosen, and a member in CodeMirror (its own document, provider and CodeMirror view): typing at once both ways, multi-line edits, undo and redo of one's own edits only (Ctrl+Z too, after a switch, where Monaco's own undo would have nothing), cursors and names both ways, a hostile name and colour, a forged cursor, the document options, switching editors in Settings and back, offline edits merging. Monaco runs in jsdom with a few stand-ins: a canvas context that draws nothing, a clipboard, a `ResizeObserver` and a `Worker` that never answers. |

`dom/fakenet.mjs` is the fake peerjs network shared by the jsdom tests; it is cut from `room-test.mjs`. As in peerjs, closing one end of a media call leaves the other end open. `dom/fakeopfs.mjs` is a small
