...
app/ui/                 dom.js (h() builder, icons, dialogs, toasts), layout.js (bottom tabs or desktop panels), qr.js, start-view.js, settings-view.js, styles.css
vendor/                 peerjs 1.5.5 UMD (window.Peer), qrcode.js, editor.js (CodeMirror 6 + Yjs ES module bundle), dockview.js + .css (dockview-core 8.2.0), words.js (BIP-39 list)
...
- **PWA and the share target:** `sw.js` is a classic worker (Firefox has no module workers) and serves network first with the cache as fallback, so an update is never held back; its `SHELL` must list every file under `app/` and `vendor/` except `vendor/editor.js` and `vendor/dockview.*`, which are cached on first use (the Editor opened, a wide window). An Android share is a POST that a static host can't answer: the worker stores the files and text in the `peerkit-share` cache and redirects to `./?share=1`; `app/share.js` reads that cache once, and only when a room is open, so the share survives the reload that opening a room does. `ctx.onShare(fn)` hands it to the Transfer tool. Cache storage is missing on `http://<lan-ip>`, so every call is guarded.
- **Stored data:** localStorage data is versioned (`peerkit.settings`, `.room`, `.device`, `.turn`, `.editor`, `.voice`, `.layout`); editor documents live in IndexedDB. `peerkit.rooms` / `.recent` from pairing are only checked for a one-time notice. Validate everything read from storage, links or members, because all of it is untrusted.
