...
A mobile-first static web app for peer-to-peer tools built on peerjs (WebRTC). Up to 8 devices share a room (QR code, link, 4-word code or recent rooms) and use the tools over it; the room keeps working when anyone leaves. Tools so far: Chat (messages and files with a shared history, files kept for the room, a viewer), Stream (camera and screen) and Editor (shared documents), and everyone in the room can talk over voice chat. Android Chrome and desktop browsers are the targets. The UI is in English.
...
app/roomdoc.js          RoomDoc: the room's own Y.Doc (chat and its files), IndexedDB + DocProvider on ch 'room'
...
- **Stored data:** localStorage data is versioned (`peerkit.settings`, `.room`, `.device`, `.turn`, `.editor`, `.voice`, `.layout`, `.chat`, `.files`); editor documents (`peerkit.doc:<room ID>`) and the room document (`peerkit.room:<room ID>`) live in IndexedDB, kept files in OPFS. "Leave and forget" deletes all three for the room. `peerkit.rooms` / `.recent` from pairing are only checked for a one-time notice. Validate everything read from storage, links or members, because all of it is untrusted.
